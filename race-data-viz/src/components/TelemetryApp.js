...
export default function TelemetryViewer() {
  ...
  // Fetch all available tracks on component mount
  useEffect(() => {
  ...
  // Fetch players for the selected track
  useEffect(() => {
    if (selectedTrack) {
      fetch(
        `https://dv-telemetry-load.azurewebsites.net/api/tracks/${encodeURIComponent(
          selectedTrack,
        )}`,
      )
        .then((res) => res.json())
  ...
  const metrics = Object.keys(slowData[0]).filter(
    (k) => typeof slowData[0][k] === "number" && !["Lon", "Lat"].includes(k),
  );
  const metricExtent = d3.extent(
    [...slowData, ...fastData],
    (d) => d[selectedMapMetric],
  );
  const colorScale = d3
    .scaleLinear()
    .domain([
  ...
  const formatRange = (min, max) =>
    `${Math.round(min)}–${Math.round(max)} ${getUnits(selectedMapMetric)}`;
        ...
        >
          <svg width={width} height={height}>
            {(() => {
              ...
              const [x2, y2] = project(
                slowData[slowData.length - 1].Lon,
                slowData[slowData.length - 1].Lat,
              );
              return (
                <line
                  x1={x1}
  ...
  useEffect(() => {
    ...
    if (xAxisRef.current) {
      const xAxis = d3
        .axisBottom(x)
        .ticks(5)
        .tickFormat((d) => `${(d * 0.05).toFixed(1)}s`);
    ...
    >
      ...
      {hoverIndex !== null && (
        <>
          <circle
            cx={x(hoverIndex)}
            cy={y(slowData[hoverIndex]?.[metric])}
            r={4}
            fill="blue"
          />
          <circle
            cx={x(hoverIndex)}
            cy={y(fastData[hoverIndex]?.[metric])}
            r={4}
            fill="green"
          />
        </>
